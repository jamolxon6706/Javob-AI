...
| 24 | Analytics endpoints fetch a period's rows once and aggregate in Python, instead of DB-side `date_trunc`/`GROUP BY` | The API test suite runs on SQLite (see `apps/api/tests/conftest.py`), which doesn't support Postgres date functions; a tenant's message volume is small enough that this is cheap in production too. Revisit with real DB-side aggregation if a tenant's volume ever makes this a bottleneck | 2026-06-30 |
| 25 | Sentiment tagging is a keyword list (UZ+RU), not a model call | Only needs to catch clearly angry/frustrated customers for auto-escalation — a false negative just means the normal RAG/LLM pipeline runs as before; a false positive means an early (safe-direction) handoff. A model call would add latency and cost to every single inbound message just to catch a minority case | 2026-06-30 |
| 26 | Angry-customer detection runs before RAG/LLM, inside `CoreEngine.process()`, not as a separate pipeline stage | Keeps the single-source-of-truth decision pipeline promise from the architecture doc; an angry customer shouldn't wait on a RAG/LLM round-trip the bot was never going to be trusted for anyway | 2026-06-30 |
| 27 | The AI eval harness (`worker/services/eval.py`) checks RAG *retrieval* (which FAQ, or "not found"), not full LLM-generated answer text | Grounded-answer quality depends on the live LLM provider's behavior, which isn't repeatable/pinnable in a harness; retrieval regressing in the harness means retrieval regressed in production too, since it's the same `RAGService` call. `expected_answer_contains` cases still exercise the low/high-threshold routing decision without spending an LLM call on every harness run | 2026-06-30 |
| 28 | Groq's `cost_usd` is hardcoded to `0.0` on every `EngineReply` | Groq's free tier is the only LLM provider wired up today (see Decision #4); the field is real and persisted so a future paid escalation model just needs to report a non-zero figure — no schema change needed later | 2026-06-30 |
| 29 | "Top questions" / "FAQ gaps" use exact-match text normalization (lowercase, strip punctuation, collapse whitespace), not semantic clustering | Re-embedding every inbound message just to build a dashboard panel would double the RAG cost of production traffic for an analytics-only feature; good enough to group obvious repeats. Revisit if tenants want true paraphrase clustering | 2026-06-30 |
...
- **The AI eval harness checks retrieval only, not full LLM-generated answer text** — see Decision #27. A regression in LLM phrasing/tone (as opposed to which FAQ gets matched) won't be caught.
- **"Top questions" / "FAQ gaps" clustering is exact-match text normalization, not semantic** — see Decision #29. Two different phrasings of the same question are counted separately.
- **`by_handoff_reason` in `/analytics/overview` reflects each conversation's *current* `handoff_reason`, not a full historical log** — if a conversation was handed off for `low_confidence` and later re-handed-off for `angry_customer`, only the latest reason is counted for the period.
...
- [x] Phase 13 — Analytics + observability + AI QA
...
## Phase 13 — Analytics, Observability & AI Eval Harness

### What was added

**Backend — per-answer audit trail**
- `messages` gained `model`, `latency_ms`, `cost_usd`, `sentiment` columns; `conversations` gained `handoff_reason`.
- `CoreEngine.process()` now measures latency around each pipeline stage (RAG search, LLM call, action execution) and returns it on `EngineReply`, along with which model answered and why a handoff happened (`low_confidence` | `angry_customer` at the engine level; `out_of_window` | `rate_limited` are still set by the dispatcher).
- `worker/services/sentiment.py` — keyword-based UZ/RU sentiment tagging. An angry-worded inbound message short-circuits straight to handoff, before any RAG/LLM call (see Decision #25, #26). Every inbound message (all four channels) is tagged, angry or not, so `angry_count` in analytics reflects total volume, not just escalations.

**Backend — AI eval harness (`apps/api/src/javobai/db/models/eval.py`, `worker/services/eval.py`)**
- `EvalCase` — a tenant-authored golden test set: a question plus either an `expected_faq_id` or an `expected_answer_contains` substring.
- `EvalHarness.run()` re-embeds each question and runs it through the same `RAGService` used in production, records pass/fail per case, and flags `is_regression` when the pass rate drops more than `REGRESSION_DROP_THRESHOLD` (10 points) vs. the previous run.
- `worker/tasks/eval.py::run_eval_job` — ARQ task triggered from the dashboard's "Run eval" button.

**Backend — analytics API (`apps/api/src/javobai/analytics/`)**
- `GET /analytics/overview` — reply/handoff rate, message volumes, avg response time, angry-customer count, breakdown by source (faq/llm/action/flow/handoff/operator) and by channel, and handoff-reason counts.
- `GET /analytics/timeseries` — daily inbound/outbound/handoff volume for the chart.
- `GET /analytics/top-questions` — most frequent inbound questions, and "FAQ gaps" (frequent questions from conversations that ended in a handoff).
- `GET /analytics/quality`, `/analytics/eval-cases` (CRUD), `POST /analytics/eval-cases/run` — the eval harness dashboard panel.

**Migration** `013_analytics_eval.py` — the columns above, plus `eval_cases`, `eval_runs`, `eval_results` tables.

**Frontend (`apps/web/src/components/analytics/analytics-client.tsx`)**
- Three tabs: Overview (KPI cards, a lightweight inline-SVG timeseries chart, source/channel/handoff-reason breakdowns — no chart library dependency was added, see Decision below), Questions (top questions + FAQ gaps), Quality (eval run history, regression banner, golden-test-set CRUD, "Run eval" button).
- Full i18n for uz + ru (`messages/{uz,ru}.json` → `analytics` namespace).

### Decisions specific to this phase
See Decisions table entries #24–#29 (Python-side date bucketing for SQLite test compatibility, keyword-based sentiment, angry-escalation ordering, eval harness scope, Groq cost tracking, and top-questions clustering approach).

One more not in the table: **no chart library (recharts etc.) was added** to `apps/web` — the timeseries chart is a ~30-line inline SVG component. Pulling in a new dependency for one line chart wasn't worth the install-size/lockfile churn; revisit if Phase 13's dashboard grows more chart types.

### Verification
- `apps/worker/tests/test_sentiment.py`, `test_eval.py`, and additions to `test_rag.py` (angry-customer escalation, audit-field population) — 79/79 worker tests pass.
- `apps/api/tests/test_analytics.py` — overview/timeseries/top-questions math, tenant isolation, eval-case CRUD, eval-run triggering — 88/88 API tests pass.
- `apps/web` — zero new TypeScript errors (`tsc --noEmit`), 50/50 existing vitest tests still pass.
